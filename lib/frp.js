...
async function childFrpc() {
  ...
  frp.stdout.on("data", async data => {
    ...
    if (data.indexOf("start proxy success") > -1) {
      const protocol = process.env.FRP_SERVER === 'home.esonwong.com' ? 'https' : 'http'
      const url = `${protocol}://${process.env.FRP_SERVER}:${process.env.FRP_REMOTE_PORT}`;
      const text = `穿透成功！使用 ${url}  访问遥控车控制界面`;
      console.info(text);
      TTS(`穿透成功！使用 ${protocol} 协议、${process.env.FRP_REMOTE_PORT}端口访问 ${process.env.FRP_SERVER} 进入遥控车控制界面。`, { reg: "1" });
