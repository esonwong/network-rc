...
module.exports = class Camera {
  constructor(options) {
    ...
    getCameraFormats(devPath).then((v) => {
      ...
      logger.info(`${name}格式列表`, this.formatList);
    });
    ...
    logger.info(`Camera ${this.cameraName} websocker server starting`, path);
    const wss = (this.wss = new WebSocketServer(
      ...
      () => {
        logger.info(`Camera ${this.cameraName} websocker server started`, path);
      }
    ));
    ...
    wss.on("connection", async (socket) => {
      logger.info(`客户端已连接 Camera ${this.cameraName}`);
  ...
  async open({ width = 400, inputFormatIndex, fps }) {
    ...
    if (this.streamer) {
      logger.info(`Camera ${this.cameraName} ffmpeg streamer already open`);
      this.close();
    }

    logger.info(`Camera ${this.cameraName} , 输出分辨率:${width}x${height}`);
  ...
  messageHandle(data, type, transport) {
    ...
    logger.info(`Camera ${this.cameraName} 收到 ${type} 的 message`, payload);
    switch (action) {
      case "open-request":
  ...
  close() {
    if (this.streamer) {
      logger.info(`Camera ${this.cameraName} ffmpeg streamer killing`);
  ...
  broadcast(action, payload) {
    logger.info(`Camera ${this.cameraName} broadcase`, action, payload);
