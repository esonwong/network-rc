...
class AudioPlayer extends EventEmitter {
  ...
  async play({ type, data }) {
    ...
    switch (type) {
      ...
      case "mp3 file path":
        ...
        const pcmPath = `/var/audio/${filename.replace("mp3", "pcm")}`;
  ...
  async volume(v) {
    console.log("设置音量", v);
    await exec(
      `amixer cset numid=1,iface=MIXER,name='PCM Playback Volume' ${v}`
    );
    this.getVolume();
  }
  ...
  async getVolume() {
    ...
    const match = / values=(\-*\d+)/gi.exec(stdout);
    if (match) {
