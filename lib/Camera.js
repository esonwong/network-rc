      ...
      socket.on("message", (m) => {
        ...
        switch (action) {
          case "resize":
            ...
            if (this.size.width === payload.width) return;
            console.log(`Camera ${cameraIndex} 分辨率切换到 ${JSON.stringify(payload)}`);
            this.open(payload);
            break;
          default:
        }
      ...
      v4l.on('exit', async () => {
        for (let index = 0; index < devList.length; index++) {
          ...
          await new Promise(resolve => {
            const v4l = spawn('v4l2-ctl',[`--device=${item.dev}`,'--all'])
              v4l.stdout.on('data', (data) => {
                ...
                const [width,height] = /Width\/Height\s*\:\s*(\d+\/\d+)/ig.exec(outText)[1].split('/')
