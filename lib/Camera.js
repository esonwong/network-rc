...
module.exports = class Camera {
  constructor(options) {
    ...
    getCameraFormats(devPath).then(v => {
      this.formatList = v.filter(({ format }) => ['yuyv422', 'mjpeg', 'h264'].includes(format));
    ...
    wss.on("connection", async (socket) => {
      ...
      socket.on("message", (m) => {
        ...
        switch (action) {
          case "open-request":
            let { width, inputFormatIndex, fps = 30 } = payload
            if (inputFormatIndex === undefined) {
              const index = this.formatList.findIndex(({ format }) => format === 'mjpeg') 
              inputFormatIndex = index < 0 ?  0 : index
            }
            this.open({ width, inputFormatIndex, fps });
            socket.send(JSON.stringify({
              action: "open", payload: { inputFormatIndex, fps }
            }));
            break;
          default:
        }
