...
module.exports = class Camera {
  constructor(options) {
    ...
    const { cameraIndex, cardType, name, deviceSize, server, devPath } = options;
  ...
  async open({ width = 400 }) {
    ...
    try {
      this.isMjpegSupported = (await getCameraFormats(devPath)).includes('mjpeg')
    } catch (e) {
      console.error(`${devPath} 获取摄像头格式错误`, e)
    }
    ...
    this.streamer = ffmpeg(this.options.deviceSize, { width, height }, this.options.devPath, this.isMjpegSupported);
...
const getCameraFormats = async function (devPath) {
  const result = await asyncCommand(`ffmpeg -f v4l2 -list_formats all -i ${devPath}`, 'stderr')
  // console.log(`${devPath} 摄像头格式, ${result}`)
  return result
}

const ffmpeg = function (deviceSize, outSize, input, mjpeg = false) {
  const fps = 30;
