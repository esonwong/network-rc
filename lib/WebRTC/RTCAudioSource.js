...
class NodeWebRtcAudioSource extends RTCAudioSource {
  ...
  async start() {
    ...
    console.log("WebRTC Audio Source Start");
    this.command = ffmpeg("plughw:1,0")
      ...
      // })
      .on("end", function () {
        console.log("WebRTC Audio Source processing finished !");
      });
    ...
    this.ffstream.on("drain", () => {
      console.log("WebRTC Audio  stream drain");
    });
    this.ffstream.on("error", () => {
      console.log("WebRTC Audio  stream error");
    });
    this.ffstream.on("finish", () => {
      console.log("WebRTC Audio  stream finish");
    });
